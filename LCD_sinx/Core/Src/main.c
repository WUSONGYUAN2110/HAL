...
static void draw_sine_wave_time(float time_offset, uint16_t color)
{
  ...
  /* 使用静态变量保存上一帧波形数据用于擦除 */
  static int16_t prev_wave_y[LCD_WIDTH];  /* 存储上一帧每个x对应的y值 */
  ...
  /* 绘制横坐标时间标签（动态，每帧更新） */
  const int x_ticks = 5;
  static char prev_labels[5][16] = {0};  /* 保存上一帧的标签，只在变化时重绘 */
  for (int i = 0; i < x_ticks; i++) {
    float frac = (float)i / (float)(x_ticks - 1);
    ...
    char lbl[16];
    snprintf(lbl, sizeof(lbl), "%.1fs", t_label);
    
    /* 只在标签值变化时才重绘，避免频闪 */
    if (strcmp(prev_labels[i], lbl) != 0) {
      lcd_fill(tx, drawable_y1 + 2, tx + 39, drawable_y1 + 14, WHITE);
      lcd_show_string(tx, drawable_y1 + 2, 40, 12, 12, lbl, BLACK);
      strncpy(prev_labels[i], lbl, sizeof(prev_labels[i]) - 1);
    }
  }
}
